...
public class PostOrderTraversalWithoutRecursionUsingTwoStacks {
	...
	private static void postOrderTraversal(BinaryTreeNode root) {
		Stack< BinaryTreeNode > processStack = new Stack<>();
		Stack< BinaryTreeNode > auxillaryStack = new Stack<>();
		...
		while( !processStack.isEmpty() || current != null ) {
			...
			if( current != null ) {
				...
				if( current.right != null && ( auxillaryStack.isEmpty() || 
						auxillaryStack.peek().data != current.data) ) {
					processStack.push( current );
					auxillaryStack.push( current );
					current = current.right;
					continue;
				} else if( !auxillaryStack.isEmpty() && 
						current.data == auxillaryStack.peek().data ) {
					auxillaryStack.pop();
				}
